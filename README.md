...
Балансировщик выступает HTTP Reverse Proxy. Вычитывается весь HTTP запрос, исходя из запроса перенаправляется на нужный сервер. Будет использоваться **Nginx**. Надежность сопоставима с надежностью железа. Будет использоваться для кэширования статики, сжатия контента.
...
Объем = RPS * 1 Кб
Ядра CPU = RPS * 0,75 мс = RPS × 0,00075 с. Это будет требуемым количеством «ядровых секунд» в секунду, что равносильно количеству ядер.

| Датацентр       | RPS    | Объем данных МБ/с | Требуемые ядра CPU |
|-----------------|--------|-------------------|--------------------|
| Москва          | 63 245 | 61,8              | 47,4               |
| Санкт-Петербург | 23 322 | 22,8              | 17,5               |
| Ростов-на-Дону  | 31 096 | 30,4              | 23,3               |
| Екатеринбург    | 38 869 | 38                | 29,2               |
| Новосибирск     | 23 322 | 22,8              | 17,5               |
