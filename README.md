...
Чтобы следить за состоянием хостов, необходим сервис **Keepalived**. Keepalived обеспечивает отказоустойчивость балансеров с помощью **VRRP** (Virtual Router Redundancy Protocol) механизма резервирования нод между собой (берётся в два раза больше балансеров чем для покрытия пиковой нагрузки, один ip для двух машин, но анонсируется только на одной машине, в зависимости от того, какая работоспособна). Следит за состоянием кластера, меняет конфигурацию балансировщика, исключая нерабочие хосты.

#### L7-балансировщик

Балансировщик выступает HTTP Reverse Proxy. Вычитывается весь HTTP запрос, исходя из этого перенаправляется на нужный сервер. Будет использоваться **Nginx**. Надежность сопоставима с надежностью железа. Будет использоваться для отдачи статики, сжатия контента.
...
Nginx выполняет повторный запрос в случае падения бэкенда, перезапускать сервер без downtown.

#### Управление сервисами

В каждом датацентре развернут кластер Kubernetes. Kubernetes занимается запуском экземпляров сервисов и регистрацией их в реестре (зарегистрированы будут только экземпляры, прошедшие readiness probe). Оркестрация. Также он производит auto-scaling – количество экземпляров выбирается под текущую нагрузку. 
