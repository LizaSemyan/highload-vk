...
Трафик на пост (средние значения):
- 5 КБ аватарка сообщества/профиля
- 250 КБ фото в посте. Постов с фото 64%.
- 1 символ кодируется 1 байтом. В среднем в посте 250 символов. Тогда это 250 байт. Постов только с текстом ~16%.
- 50 КБ превью видео в посте. Постов с видео ~20%.
