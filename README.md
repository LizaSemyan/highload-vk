...
#### Этап 0. Сбор данных и построение эмбеддингов
...
Помимо оценок учитывается так же и опубликованный контент:

- Тематики постов
- Тексты
- Картинки

**Задача**: на основе всех вышеописанных данных описать пользователя (в том числе и сообщество) набором **векторов**, которые будут отражать его интересы.

Пересчет векторов происходит после каждой сессии пользователя. Концом сессии считается бездействие в течение 15 минут.

**Итог**: все пользователи и сообщества описаны набором векторов (~ 10 на пользователя).

#### Этап 1. Подбор кандидатов

- Берется векторное представление текущего пользователя и сравнивается с векторным представлением других пользователей и сообществ
- Чем ближе вектора находятся друг к другу, тем более схожи пользователи/сообщества

**Задача**:

- Получить 300 кандидатов из самых популярных авторов
- Получить 300 кандидатов из (user-based)
- Получить 300 кандидатов из (memory-based)

**Итог**: получены 300 релевантных кандидатов.
...
- Отбираем релевантные рекламные посты на основе профиля пользователя и его действий
