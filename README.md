...
Чтобы следить за состоянием хостов, необходим сервис **Keepalived**. Keepalived обеспечивает отказоустойчивость балансеров с помощью **VRRP/CARP** механизма резервирования нод между собой (берётся в два раза больше балансеров чем для покрытия пиковой нагрузки, один ip для двух машин, но анонсируется только на одной машине, в зависимости от того, какая работоспособна). Следит за состоянием кластера, меняет конфигурацию балансировщика, исключая нерабочие хосты.

#### L7-балансировщик

Вычитывается весь HTTP запрос, исходя из этого перенаправляется на нужный сервер. Будет использоваться **Nginx**. Надежность сопоставима с надежностью железа. 
...
- Гибкие настройки для учета бизнес-логики (например, учет заголовков, url, геолокации и др)
