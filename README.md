...
|---------|------------------------|-----------|
| post    | idx_post_author_created_at (author_id, created_at), idx_post_community_created_at (community_id, created_at) | Быстрый поиск постов друзей и сообществ c сортировкой по дате |
...
| knowledge-based | Формирование пула рекламных постов для ленты | Для показа персонализированной рекламы |
...
Явные оценки хранятся в базе как сущности (лайк, подписка, комментарий). Неявные оценки хранятся в таблице user_activity (тип активности хранится в таблице activity_type)

Построение матрицы пользователь-пост на основе явных и неявных оценок пользователя для коллаборативной фильтрации для выявления паттернов.
...
На данном этапе можно интегрировать машинное обучение для оценки релевантности постов. Используя модели факторизации матриц или нейронные сети, можно предсказывать вероятность того, что пользователь заинтересуется конкретным контентом. Это позволит улучшить качество ранжирования.
...
Также необходимо учесть дату публикации поста, более новые посты будут выше. Для этого при ранжировании более новым постам будет присвоен более высокий вес.

Отсекаем около 1000 постов, они отправляются на следующий этап.

#### Этап 4. Персонализация, диверсификация

- Удаление дубликатов (полность одинаковых постов)
...
#### Этап 1. Подбор рекламы

- Отбираем релевантные рекламные посты на основе профиля пользователя и его действий (используем knowledge-based алгоритм)

#### Этап 2. Встраивание в ленту
...
#### Этап 1. Определение популярности:

- Метрика популярности: Рассчитываются показатели, такие как число лайков, комментариев, просмотров и взаимодействий за последний период (например, за последние 4 часа)
- Топ-N выборка: На основе этих показателей формируется список самых популярных постов

#### Этап 2. Сохранение в кэше

- Используется Redis для хранения данных популярных постов.
- Устанавливается TTL (time-to-live) для каждого кэшированного объекта

#### Этап 3. Обновление кэша

- При достижении пороговых значений активности (низкая активность) происходит пересчёт метрик популярности и обновление кэша

#### Этап 4. Выдача из кэша

При запросе ленты пользователя сначала происходит проверка кэша на наличие популярных постов.
Если данные есть — они сразу возвращаются, снижая нагрузку на основную базу данных.
Если кэш пуст или данные устарели, происходит выборка из базы с последующим обновлением кэша.
