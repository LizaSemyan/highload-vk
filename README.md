...
| **friend**        | Хранит информацию о дружеских связях между пользователями. Каждая "дружба" хранится двумя записями | user_id UUID (идентификатор пользователя), friend_id UUID (идентификатор друга), created_at timestamp (дата и время добавления в друзья) |
...
| **friend**           | 40 | 16 - user_id, 16 - friend_id, 8 - created_at  | 726,39 млн (всего пользователей) * 121,5 (в среднем у пользователя друзей) = 88 256 млн| 3 288 ГБ |
...
| **friend**           | Добавление в друзья              | 395                       | 74                 |
...
**Задача**: на основе всех вышеописанных данных описать автора (пользователя/сообщество) набором **векторов**, которые будут отражать его интересы и направленности.
...
- После ранжирования взять срез первых 300 кандидатов
...
**Итог**: получены 300 релевантных рекомендованных кандидатов, получены подписки на авторов (пользователей и сообщества) = в среднем в сумме это 550 кандидатов.
...
- Количество запрашиваемых постов зависит от частоты публикации постов автором, степени близости автора текущему пользователю (score присвоенный на этапе отбора кандидатов)
...
- При запросе на получение ленты, пользователю из кэша на клиенте отдается последний пост из предыдущей сессии (для того чтобы пользователю не пришлось ждать прогрузку ленты)
- Проверяется кэш на бэке: если в кэше лежит лента - берем оттуда, если нет - отправляется запрос на сервис формирования ленты (предыдущие этапы алгоритма)
...
| Redis                      | Хранение рекомендаций            | Для быстрой выдачи персонализированных рекомендаций. Мгновенное получение данных с минимальными задержками|
...
| Nginx (L7)                 | Active-Passive (Keepalived + VRRP) | Горизонтальное масштабирование через Kubernetes, кэширование, Ingress-контроллер | При сбое одного инстанса балансировщика трафик автоматически перенаправляется на резервный |
...
Маршрутизация медиазапросов (глобальная + локальная балансировка:

- BGP Anycast для распределения в CDN;
- Nginx для локальной балансировки.

От API запросы идут к микросервисам. Были выделены следующие микросервисы:

- **Auth Service**. Отвечает за регистрацию, аутентификацию и авторизацию пользователей. Обращается к **User Service** для создания нового пользователя;
- **User Service**. Управление учетными записями, профилями пользователей, друзьями, заявками в друзья. Обращается к **Media Service** для получения и записи медиаконтента;
- **Community Service**. Создание и редактирование сообществ, управление участниками. Обращается к **Media Service** для получения и записи медиаконтента;
- **Feed Service**. Построение новостной ленты. Обращается к **Recommendation Service** для получения id рекомендованных постов, к **Post Service** для получения полной информации о постах;
...
- **Reaction Service**. Лайки, комментарии, активность пользователей;
- **Recommendation Service**. Формирование персонализированных рекомендаций на основе векторных представлений и коллаборативной фильтрации. Обращается к **User Service** для получения подписок на друзей и списка авторов, к **Community Service** для получения подписок на сообщества и списка авторов, к **Post Service** для получения постов от авторов, к **Reaction Service** для получения пользовательской активности.
