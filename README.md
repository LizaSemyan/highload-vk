...
|----------------------------|------------------------------------|-------------------------------|-----------|
| LVS (L4)                   | Active-passive (VRRP)              | Keepalived | Используется для первичного распределения TCP/UDP трафика |
...
| PostgreSQL                 | Репликация (master-slave)          | PgBouncer, HAProxy            | Для каждой реплики настроен фейловер. Данные критичны (учётные записи, дружбы, заявки) → необходима сильная консистентность |
...
| ClickHouse                 | Репликация                         |  | Используется для аналитики и хранения пользовательских действий. Репликация и шардирование обеспечивают отказоустойчивость |
| Redis                      | Репликация (master-slave)          | Redis Cluster | При падении master происходит автоматическое переключение на slave |
| S3                         | Репликация между зонами            | Версионирование, многозонное хранение, CDN-акселерация | Хранение медиаконтента (фото). Репликация на уровне облака обеспечивает геоустойчивость и целостность данных |
...
| Prometheus + Grafana       | Active-active (кластеризация)      |  | Наблюдаемость и метрики, критично для диагностики |
