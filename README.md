...
| **session**       | Хранит активные сессии пользователей для авторизации и управления доступом | id UUID (уникальный идентификатор сессии), user_id UUID (идентификатор пользователя), session_token varchar(255) (уникальный токен сессии), created_at timestamp (дата и время создания сессии), expires_at timestamp (дата и время истечения сессии) |
...
| **authors_vector** | Хранит рассчитанные эмбеддинги пользователей, используемые для рекомендаций контента | user_id UUID (идентификатор пользователя), vector float[] (эмбеддинг пользователя), calculated_at timestamp (дата и время расчета вектора) |
...
| **session**          | 303   | 16 - id, 16 - user_id, 255 - session_token, 8 - created_at, 8 - expires_at | 126 млн (по числу MAU, одна активная сессия на пользователя в месяц) | 35,56 ГБ |
...
| **authors_vector**   | 4 024 | 16 - user_id, 4000 (1000 float * 4 байта) - vector, 8 - calculated_at | 126 млн (по числу MAU, пересчитывается раз в сутки) | 472,2 ГБ |
...
| **session**          | Создание и обновление сессий         | 126                       | 49                 |
...
| **authors_vector**   | Перерасчёт эмбеддингов пользователей | 3 780                     | 1 459              |
...
| recommendation | Redis | Быстрое хранение персонализированных рекомендаций |
| authors_vector | Redis | Быстрый доступ к эмбеддингам для формирования рекомендаций |
...
| session | Шардирование в Redis по user_id с использованием Redis Cluster | Redis Cluster для высокой доступности и автоматического переключения | Необходим быстрый доступ к данным с минимальными задержками; эффективное управление временем жизни сессий (TTL) |
...
| authors_vector | Шардирование по user_id с использованием Redis Cluster | Redis Cluster для высокой доступности и отказоустойчивости | Эмбеддинги пользователей должны быть быстро доступны для рекомендаций |
...
- RPS = Лайк на пост + Комментарий = 12 794 + 485 = 13 279
...
Для Nginx на 1 CPU: 500 RPS, 10 Mb RAM
...
| Название       | Хостинг | Конфигурация                        | Cores | Cnt | Покупка                 | Аренда за 3 года  |
|----------------|---------|-------------------------------------|-------|-----|-------------------------|-------------------|
| kubernode      | own     | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s    | 64    | 36  | $522 000 (14500 * 36)   | |
| nginx          | own     | 2x6338/8x32GB/2xSSD1T/2x25Gb/s      | 64    | 4   | $58 000 (14500 * 4)     | |
| api_gateway    | own     | 2x6338/8x32GB/2xSSD1T/2x25Gb/s      | 64    | 2   | $29 000 (14500 * 2)     | |
| postgresql     | own     | 2x6378/8x32GB/2xSSD3.84T/2x10Gb/s   | 64    | 2   | $27 000 (13 500 * 2)    | |
| cassandra      | own     | 2x7453/16x32GB/4xSSD3.84T/2x25Gb/s  | 64    | 22  | $330 000 (15 000 * 22)  | |
| clickhouse     | own     | 2x7453/16x32GB/4xNVMe3.84T/2x25Gb/s | 64    | 14  | $210 000 (15 000 * 14)  | |
| redis          | own     | 1x5318/4x32GB/1xSSD960G/2x25Gb/s    | 32    | 19  | $152 000 (8000 * 19)    | |
| kafka          | own     | 2x6338/8x32GB/2xSSD1.92T/2x25Gb/s   | 64    | 8   | $116 000 (14 500×8)     | |
