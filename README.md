...
- Количество запросов в секунду (RPS): Предположим, 100 000 RPS.
- Средний размер SSL-сессии: 1 КБ.
- Процессорная нагрузка на одно SSL-соединение: ~1 ms (зависит от алгоритма шифрования и мощности CPU).

Расчёт:

- Объём данных для обработки: 100 000 RPS × 1 КБ = 100 МБ/с.
- Процессорная нагрузка: 100 000 RPS × 1 ms = 100 секунд CPU-времени в секунду.
- Для обработки такой нагрузки потребуется 100 ядер CPU.

Оптимизация:

- Session cache: Уменьшает время установления SSL-соединения за счёт кеширования сессий.
- Session tickets: Альтернатива session cache, но поддерживается не всеми браузерами.
- Perfect Forward Secrecy (PFS): Обеспечивает безопасность даже в случае компрометации приватного ключа сервера.
- Let’s Encrypt: Бесплатные SSL-сертификаты с автоматическим обновлением.
