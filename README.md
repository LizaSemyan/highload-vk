...
- Средний размер фотографии - 2,2 МБ. Предполагаем, что у среднестатистического пользователя в профиле 30 фотографий. Тогда около 66 МБ занимают фотографии в профиле.
- Предполагаем, что у среднестатистического пользователя в профиле 5 видео по 20 МБ.

| Тип                                 | Размер     |
|-------------------------------------|------------|
| Аватарка                            | 5 МБ       |
| Обложка                             | 20 МБ      |
| Текстовая информация о пользователе | ~3 КБ      |
| Загруженные фото в профиле          | 66 МБ      |
| Загруженные видео в профиле         | 100 МБ     |
| Итого                               | 191,003 МБ |
...
- Публикация поста от лица пользователя: (0,56 * 63530000) / 86400 = 411,769 
- Публикация поста от лица сообщества: (2,37 * 63530000) / 86400 = 1 742,663 
- Отправка заявки в друзья: (0,1 * 63530000) / 86400 = 73,53 
- Запрос на обновление ленты: (11,96 * 63530000) / 86400 = 8 794,199 
- Лайк на пост: (17,4 * 63530000) / 86400 = 12 794,236 
- Комментарий: (0,66 * 63530000) / 86400 = 485,299 
- Подписка на сообщество: (0,1 * 63530000) / 86400 = 73,53

Ввиду отсутствия данных считаем, что пиковое значение RPS в 2 раза больше среднего.

| Тип запроса                           | Среднее RPS | Пиковое RPS |
|---------------------------------------|-------------|-------------|
| Публикация поста от лица пользователя | 411,769     | 823,538     |
| Публикация поста от лица сообщества   | 1 742,663   | 3 485,326   |
| Отправка заявки в друзья              | 73,53       | 147,06      |
| Запрос на обновление ленты            | 8 794,199   | 17 588,398  |
| Лайк на пост                          | 12 794,236  | 25 588,472  |
| Комментарий                           | 485,299     | 970,598     |
| Подписка на сообщество                | 73,53       | 147,06      |
| Итого                                 | 24 375,226  | 48 750,452  |
