...
|----------|--------------------|-----------|
| hybrid: user-based и memory-based | Формирование рекомендательной ленты пользователя | Чем более релевантные посты показываются пользователю, тем больше своего времени и денег он вложит в продукт, так как он будет ему нравиться |
...
**Задача**: на основе всех собранных оценок описать пользователя набором **векторов**, которые будут отражать его интересы.
...
| Компонент системы          | Способ резервирования            | Пояснение |
|----------------------------|----------------------------------|-----------|
| Nginx                      |  |  |
| API Gateway                |  |  |
| PostgreSQL                 |  |  |
